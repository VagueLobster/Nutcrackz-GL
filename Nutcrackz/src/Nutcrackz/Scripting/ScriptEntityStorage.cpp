...
namespace Nutcrackz {
	...
	void ScriptStorage::InitializeEntityStorage(UUID scriptID, UUID entityID)
	{
		...
		NZ_CORE_VERIFY(scriptEngine.IsValidScript(scriptID));
		NZ_CORE_VERIFY(!EntityStorage.contains(entityID));
	...
	void ScriptStorage::CopyTo(ScriptStorage& other) const
	{
		...
		for (const auto& [entityID, entityStorage] : EntityStorage)
		{
			...
			{
				NZ_CORE_WARN("Cannot copy script data for script ID {}. The script is no longer valid", entityStorage.ScriptID);
				continue;
			}
			...
			for (const auto& [fieldID, fieldStorage] : entityStorage.Fields)
			{
				...
				{
					NZ_CORE_WARN("Cannot copy script data for field {}. The field is no longer contained in the script.", fieldStorage.GetName());
					continue;
				}
	...
	void ScriptStorage::CopyEntityStorage(UUID entityID, UUID targetEntityID, ScriptStorage& targetStorage) const
	{
		...
		{
			NZ_CORE_ERROR("Cannot copy script storage to entity {} because InitializeScriptStorage hasn't been called for it.", targetEntityID);
			return;
		}
		...
		{
			NZ_CORE_ERROR("Cannot copy script data for script ID {}. The script is no longer valid", srcStorage.ScriptID);
			return;
		}
		...
		{
			NZ_CORE_ERROR("Cannot copy script storage from entity {} to entity {} because they have different scritps!", entityID, targetEntityID);
			return;
		}
		...
		for (const auto& [fieldID, fieldStorage] : srcStorage.Fields)
		{
			if (!scriptMetadata.Fields.contains(fieldID))
			{
				NZ_CORE_ERROR("Cannot copy script data for field {}. The field is no longer contained in the script.", fieldStorage.GetName());
				continue;
			}
