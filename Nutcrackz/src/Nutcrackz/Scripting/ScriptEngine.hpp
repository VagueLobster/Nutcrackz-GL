...
namespace Nutcrackz {
	...
	{
	public:
		RefPtr<Scene> GetCurrentScene() const { return m_CurrentScene; }
		void SetCurrentScene(RefPtr<Scene> scene) { m_CurrentScene = scene; }
	...
	private:
		...
		void Initialize(RefPtr<Project> project);
		void Shutdown();
		...
		{
			NZ_CORE_VERIFY(storage.EntityStorage.contains(entityID));
		...
		void DestroyInstance(UUID entityID, ScriptStorage& storage)
		{
			...
			NZ_CORE_VERIFY(IsValidScript(entityStorage.ScriptID));
	...
	private:
		...
		RefPtr<Scene> m_CurrentScene = nullptr;
