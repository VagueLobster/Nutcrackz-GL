...
#include "UIElements/InputFloat.hpp"
...
#pragma region Input
	...
	static bool InputUInt(uint64_t& value, uint64_t clearValue = 0.0f, int min = 0, int max = 0, bool isMixed = false)
	{
		ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, isMixed);
	...
	//static bool ColorPicker(const char* id, glm::vec4& color, bool isMixed = false)
	static bool ColorPicker(const char* id, rtmcpp::Vec4& color, bool isMixed = false)
	{

		bool modified = false;
		ImVec4 col = { color.X, color.Y, color.Z, color.W };
		ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, isMixed);
		modified |= ImGui::ColorEdit4(id, &color.X);
		ImGui::PopItemFlag();

		return modified;
	}
	//static bool ColorPicker(const char* name, const char* id, glm::vec4& color, bool isMixed = false)
	static bool ColorPicker(const char* name, const char* id, rtmcpp::Vec4& color, bool isMixed = false)
	{
		ShiftY(4.0f);
		ImGui::Text("%s", name);
		ImGui::NextColumn();
...
#pragma region DragDrop
	...
	template<typename T, typename Callback>
	static bool DragSource(const Nutcrackz::AssetType& type, T* data, Callback callback)
	{
		return DragSource(Nutcrackz::AssetTypeToString(type), data, callback);
	}
	...
	template<typename T, typename Callback>
	inline static bool DropTarget(const Nutcrackz::AssetType& type, Callback callback)
	{
		bool accepted = false;
		if (ImGui::BeginDragDropTarget())
		{
			const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(Nutcrackz::AssetTypeToString(type));
			if (payload)
			{
				callback(*(T*)payload->Data);
				accepted = true;
