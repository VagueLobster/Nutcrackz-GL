...
namespace Nutcrackz {
	...
	namespace Utils {
		...
		static GLenum NutcrackzFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			NZ_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			NZ_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		//NZ_PROFILE_FUNCTION();
		NZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::NutcrackzFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
