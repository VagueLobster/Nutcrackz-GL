...
RTMCPP_EXPORT namespace rtmcpp {
	...
	struct Matrix4x4
	{
		...
		static Matrix4x4 RTMCPP_VECTOR_CALL Orthographic(ComponentType width, ComponentType height, ComponentType nearClip, ComponentType farClip)
		{
			Matrix4x4 result;
			ComponentType invDistance = ComponentType(1) / (farClip - nearClip);
			...
			result.Value.w_axis = rtm::vector_set(ComponentType(0), ComponentType(0), -invDistance * nearClip, ComponentType(1));
			return result;
		}
	};
