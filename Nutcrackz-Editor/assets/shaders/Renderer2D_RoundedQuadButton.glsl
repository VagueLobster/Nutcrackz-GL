// ==========================
// * Nutcrackz 2D *
// Renderer2D Rounded Quad Shader
// ==========================
...
layout (location = 6) in vec2 a_Dimensions;
layout (location = 7) in int a_ShouldInvertUICorners;
...
layout (location = 5) out flat float v_Radius;
layout (location = 6) out flat int v_ShouldInvertUICorners;
...
void main()
{
	...
	Output.Dimensions = a_Dimensions;
	v_TexIndex = a_TexIndex;
	v_Radius = a_Radius;
	v_ShouldInvertUICorners = a_ShouldInvertUICorners;
...
layout (location = 5) in flat float v_Radius;
layout (location = 6) in flat int v_ShouldInvertUICorners;
layout (location = 7) in flat int v_EntityID;

layout(binding = 0) uniform sampler2D u_Textures[32];

bool ShouldDiscard(vec2 texCoords, vec2 dimensions, float radius)
{
	...
	if (length(coords - circleCenter) > radius && coords.x < circleCenter.x && coords.y < circleCenter.y)
		return true;

	// 2nd circle
	circleCenter.x += dimensions.x - (2 * radius);

	if (length(coords - circleCenter) > radius && coords.x > circleCenter.x && coords.y < circleCenter.y)
		return true;

	// 3rd circle
	circleCenter.y += dimensions.y - (2 * radius);
...
void main()
{
	if (v_ShouldInvertUICorners == 0)
	{
		if (ShouldDiscard(Input.TexCoord, Input.Dimensions, v_Radius))
			discard;
	}
	else
	{
		vec2 texCoords = Input.TexCoord * Input.Dimensions;

		if (length(texCoords - vec2(0, 0)) < v_Radius || length(texCoords - vec2(0, Input.Dimensions.y)) < v_Radius ||
			length(texCoords - vec2(Input.Dimensions.x, 0)) < v_Radius || length(texCoords - Input.Dimensions) < v_Radius)
		{
			discard;
		}
	}
