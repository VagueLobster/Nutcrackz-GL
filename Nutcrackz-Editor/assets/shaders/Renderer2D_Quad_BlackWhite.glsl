...
layout (location = 5) in float a_Saturation;
...
layout (location = 4) out flat float v_Saturation;
...
layout (location = 4) in flat float v_Saturation;
...
void main()
{
	...
	vec3 finalImage = texColor.rgb;
	float lum = (finalImage.r + finalImage.g + finalImage.b) / 3.0;
	
	//float saturation = 0.0; // 0.0 = b&w, 1.0 = color
	
	finalImage = mix(vec3(dot(vec3(lum), finalImage)), finalImage, v_Saturation);
	//finalImage = mix(vec3(dot(vec3(finalImage.g), finalImage)), finalImage, v_Saturation);

	if (texColor.a == 0.0)
		discard;

	o_Color = vec4(finalImage, texColor.a);	
